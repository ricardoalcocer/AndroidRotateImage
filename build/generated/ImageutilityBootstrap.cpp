...
#define TAG "com.alcoapps.imageutility"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Imageutility_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Imageutility.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::ImageutilityBindings::lookupGeneratedInit(
...
static void Imageutility_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Imageutility_getBinding)->GetFunction());
}

static void Imageutility_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::ImageutilityBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry ImageutilityBinding = {
	"com.alcoapps.imageutility",
	Imageutility_init,
	Imageutility_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_alcoapps_imageutility_ImageutilityBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.alcoapps.imageutility", &ImageutilityBinding);
	titanium::KrollBindings::addExternalLookup(&(::ImageutilityBindings::lookupGeneratedInit));
}
